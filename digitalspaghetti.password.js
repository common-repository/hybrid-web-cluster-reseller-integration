...
digitalspaghetti.password = {	
	'defaults' : {
		...
		'colors': ["#ff3333", "#ffb033", "#eeff33", "#33FF33", "#81ff70"],
		'scores': [20, 30, 43, 50],
	...
	'attachWidget': function (element) {
		...
		output.push('<span class="password-strength-bar" style="background-color: rgb(255,255,255);"></span>');
		output.push('</div>');
		output = output.join('');
	...
	'calculateScore': function (word) {
		...
		for (var key in digitalspaghetti.password.rules) if (digitalspaghetti.password.rules.hasOwnProperty(key)) {
			...
			*/
			jQuery('.password-strength-bar').css({'display': 'block', 'width': digitalspaghetti.password.width + "%"}).text(digitalspaghetti.password.strText);
			jQuery('.password-strength-bar').animate({'background-color': digitalspaghetti.password.strColor},200);
		}
	}
};
